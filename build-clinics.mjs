...
const REFILTER = args.refilter === '1';   // re-run classify() over an existing vet-clinics.js, no API calls
...
(async () => {
  // --refilter: re-run the non-clinic filter over an existing vet-clinics.js with NO API
  // calls (name-based only, since an old file has no primaryType/businessStatus). Used to
  // clean a file produced by a pre-filter scan without paying to re-scan.
  if (REFILTER) {
    const fs = await import('node:fs');
    const raw = fs.readFileSync(OUT, 'utf8');
    const m = raw.match(/^window\.VET_CLINICS=(\[[\s\S]*\]);/m); // ^…/m so it hits the assignment line, not the "Format:" comment
    if (!m) { console.error(`✖ Could not find window.VET_CLINICS in ${OUT}`); process.exit(1); }
    const arr = JSON.parse(m[1]);
    console.log(`Refiltering ${arr.length} existing clinics in ${OUT} (name-based, no API calls)…`);
    await writeClinics(arr);
    return;
  }
  ...
  await writeClinics([...byId.values()]);
})();
