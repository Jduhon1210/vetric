...
// (count + names; best-effort grad years). Two-step: homepage → "Our Doctors/Team" page.
...
//
// Output → vet-staff.js:
//   window.VET_STAFF = { "<round(lat*1000)>_<round(lon*1000)>": {n, vets:[names], years?, src} }
...
function extractVets(text) {
  ...
  return { vets: [...seen.values(), ...surVets], years: [...years].sort() };
}
...
async function crawl(c) {
  ...
  if (best.vets.length && best.vets.length <= 40) {
    hit++;
    result[key(c)] = { n: best.vets.length, vets: best.vets, ...(best.years.length ? { years: best.years } : {}), src: bestSrc };
  }
...
function flush(final) {
  fs.writeFileSync(OUT,
    ...
    `// key = "<round(lat*1000)>_<round(lon*1000)>". value = {n: count, vets:[names], years?:[grad years], src}\n` +
